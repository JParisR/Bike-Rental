package es.udc.ws.bikes.adminClient.service.rest.json;
...
public class JsonAdminClientBikeDtoConversor {
	...
	public static AdminClientBikeDto toAdminClientBikeDto(InputStream jsonBike) throws ParsingException {
		try {
			...
			if (rootNode.getNodeType() != JsonNodeType.OBJECT) {
				...
				return toAdminClientBikeDto(rootNode);
			}
	...
	public static List<AdminClientBikeDto> toAdminClientBikeDtos(InputStream jsonBikes) throws ParsingException {
		try {
			...
			if (rootNode.getNodeType() != JsonNodeType.ARRAY) {
				...
				for (JsonNode BikeNode : BikesArray) {
					BikeDtos.add(toAdminClientBikeDto(BikeNode));
				}
	...
	private static AdminClientBikeDto toAdminClientBikeDto(JsonNode BikeNode) throws ParsingException {
		if (BikeNode.getNodeType() != JsonNodeType.OBJECT) {
			...
			return new AdminClientBikeDto(BikeId, description, startDate, price, units, numberOfRates,
					avgRate);
		}
	}
